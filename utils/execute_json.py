    ...
    def run(self):
        try:
            for file_settings in self.settings.get('files', []):
                ...
                time_frames = file_settings['time_frames']
                ...
                if not file_path or not time_frames:
                    self.log_error(message=f"file_path и time_frames обязательны. Ошибка в файле: {file_settings}")
                    return
    ...
    def process_file(
        ...
        *,
        ...
        if method_name == 'stft':
            ...
            self.processed_files[method_name][key] = self.stft_results_handle(stft=stft, file_path=file_path, channel=channel, freq_frame=(min_freq, max_freq), result=result)
